...
export const Resizable = () => {
  ...
  function makeResizable(element, minW = 100, minH = 100, size = 20) {
    console.log("element", element, minW, minH, size);
    // const top = document.createElement('div');
    // top.style.width = '100%';
    // top.style.height = size + 'px';
    // top.style.backgroundColor = 'transpart';
    ...
    function resizeYPositive() {
      let offsetY;
      function dragMouseDown(e) {
        console.log("event of the drag mouse down", e.button, window.e);
        if (e.button !== 0) return;
        e = e || window.Event;
        e.preventDefault();
        console.log("the clientY", e.clientY);
      ...
      function elementDrag(e) {
        ...
        console.log(">>>>>>>>>>>>>>>>>>>>>>>>> offsetY",clientY,element.offsetTop,offsetY,    y)
        console.log("offsetY y-valuue", y, "min height", minH, offsetY);
        console.log(y < minH);
        if (y < minH) {
          console.log("minH", minH, "y", y);
          y = minH;
          console.log("minH--after", minH, "y--after", y);
        }
  ...
  return (
    <div className="w-full h-screen pt-[100px]">
      ...
      <div
        id="element"
        className="  bg-[ #f1f1f1] relative bg-[white] border-[1px] border-[#d3d3d3] left[40px]  top[40px] w-[100px] h-[100px] rounded-[5px]"
      ></div>
      <div className="h-[500px]"></div>
    </div>
  );
